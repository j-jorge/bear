...
/**
 * \file
 * \brief Implementation of the bf::action_set_y_easing class.
 * \author Julien Jorge
 */
 ...
 */
bf::action_set_y_easing::action_set_y_easing
( snapshot* s, const mark* m, const bear::easing& e )
  : m_snapshot(s), m_mark(m), m_easing(e)
{

} // action_set_y_easing::action_set_y_easing()

/*----------------------------------------------------------------------------*/
void bf::action_set_y_easing::execute( gui_model& mdl )
{
  ...
  const bear::easing e(p.get_y_easing());

  p.set_y_easing(m_easing);

  m_easing = e;
} // action_set_y_easing::execute()

/*----------------------------------------------------------------------------*/
void bf::action_set_y_easing::undo( gui_model& mdl )
{
  /* the first call to execute saved the initial label and animation. */
  execute(mdl);
} // action_set_y_easing::undo()

/*----------------------------------------------------------------------------*/
bool bf::action_set_y_easing::is_identity
( const gui_model& mdl ) const
{
  const mark_placement& p = m_snapshot->get_placement(m_mark);

  return p.get_y_easing() == m_easing;
} // action_set_y_easing::is_identity()

/*----------------------------------------------------------------------------*/
wxString bf::action_set_y_easing::get_description() const
{
  return _("Set easing function");
} // action_set_y_easing::get_description()
